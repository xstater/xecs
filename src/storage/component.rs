use std::{ops::Range, any::{TypeId, type_name}};

use xsparseset::{SparseSet, SparseStorage};

use crate::{EntityId, ComponentAny, Component};
use super::Storage;
    ...
    S: SparseStorage<EntityId = EntityId> + Send + Sync,
{
    ...
    fn as_group_storage_ref(&self) -> Option<&dyn super::GroupStorage> {
        None
    }

    fn as_group_storage_mut(&mut self) -> Option<&mut dyn super::GroupStorage> {
        None
    }
}
...
impl dyn ComponentStorage {
    ...
    pub fn insert<T: Component>(&mut self, entity_id: EntityId, data: T) -> Option<T> {
        let type_id = TypeId::of::<T>();
        if type_id != self.component_type() {
